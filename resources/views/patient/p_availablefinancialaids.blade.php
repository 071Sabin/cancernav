...
@section('patientcontent')
    ...
    <div class="container bg-gray-200 my-10 p-6 rounded-xl mx-auto border-slate-400">
        <h1 class="text-gray-900 font-bold text-2xl">Available Financial Aid Providers</h1>
        <p class="text-gray-900 text-sm"><em>Total Financial Aid Providers: {{ count($financialaid_details) }}</em></p>
        ...
        @if (count($financialaid_details) > 0)
            ...
            <div class="overflow-x-auto rounded">
                <table class="w-full text-center text-gray-900 rounded bg-gray-200 table-auto text-xs mt-10">
                    <thead>
                        <tr class="uppercase bg-slate-800 text-white border-b border-slate-400">
                            ...
                            <tr class="hover:bg-gray-100">
                                ...
                                <td class="border border-slate-400 p-2 font-semibold text-slate-600 hover:underline">
                                    <a class="text-blue-600 underline"
                                        href="{{ route('patient.individualfinancialaiddetails', $financialaid->id) }}">{{ $financialaid->name }}</a>
                                </td>
                                ...
                                <td class="border border-slate-400 p-2">
                                    <button type="button"
                                        class="toggle-details bg-slate-600 hover:bg-gray-100 hover:text-black hover:border hover:border-black text-white py-1 px-3 rounded text-xs"
        ...
        @else
            <div class="mt-10 text-center">
                <p class="text-center rounded-md py-2 mt-2 text-sm bg-slate-600 text-gray-100">
                    <em>No Financial Aid Providers available! Request Navigator to add Financial Aid Provider...</em>
                </p>
            </div>
        @endif
    </div>
