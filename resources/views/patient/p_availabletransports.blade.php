...
@section('patientcontent')
    ...
    <div class="bg-gray-200 my-10 p-6 rounded-xl">
        <h1 class="text-gray-900 font-bold text-2xl">Available Transports</h1>
        <p class="text-gray-600 text-sm"><em>Total Transports Available: {{ count($transports) }}</em></p>
        ...
        @if (count($transports) > 0)
            <table class="w-full text-center text-slate-700 table-auto text-xs mt-10">
                <tr class=" bg-slate-800 text-white border-b border-gray-100 uppercase">
                    <th class=" py-2">#</th>
                ...
                @foreach ($transports as $index => $transport)
                    <tr class="border-b border-slate-600 bg-white hover:bg-gray-100">
                        ...
                        <td class="border-l border-slate-600 font-semibold underline text-blue-600 hover:text-blue-500"><a
                                href="{{ route('patient.individualtransportdetails', $transport->id) }}">{{ $transport->name }}</a>
                        </td>
