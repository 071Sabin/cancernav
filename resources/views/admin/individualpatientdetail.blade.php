...
@section('admincontent')
    <div class="container mx-auto my-10 p-6 bg-teal-700 shadow-lg rounded-lg">
        <h2 class="text-3xl font-bold text-teal-300 mb-4">Patient Details</h2>

        <div class="flex flex-col md:flex-row items-center md:items-start space-x-6">
            <!-- Profile Picture -->
            <div
                class="w-40 h-40 flex items-center text-white justify-center rounded-full overflow-hidden border-4 dark:border-black dark:border-4 border-teal-500">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full border border-teal-600 border-collapse text-sm mt-5 lg:mt-0">
                    <tbody class="text-teal-300">
                        ...
                        <tr class="border border-teal-600">
                            <th class="px-3 py-1 text-left font-semibold underline">Navigator Name</th>
                            <td class="px-3 py-1 text-white">{{ Str::title($individualpatientdetails->navigator->name) }},
                                {{ $individualpatientdetails->navigator->navigatorId }}
                                ({{ $individualpatientdetails->navigator->id }})
                            </td>
                        </tr>
                        <tr class="border border-teal-600">
        ...
        <div class="mt-6 p-4 bg-teal-800 rounded-lg overflow-x-auto">
            <h3 class="text-2xl font-bold text-teal-300 mb-3">Medical Information</h3>
            <table class="w-full lg:w-fit table-auto border border-teal-600 text-teal-300 text-xs lg:text-sm">
                <tbody>
                    <tr class="border-b border-teal-600">
                        <td class="py-2 px-3 font-medium border-r border-teal-600">Cancer Type</td>
                        <td class="py-2 px-3">{{ $individualpatientdetails->cancer_type }}</td>
                    </tr>
                    <tr class="border-b border-teal-600">
                        <td class="py-2 px-3 font-medium border-r border-teal-600">Treatment Closed</td>
                        <td class="py-2 px-3">
                            @if ($individualpatientdetails->treatment_closed === 0)
                                NO
                            @else
                                Yes on
                                {{ \Carbon\Carbon::parse($individualpatientdetails->treatment_closed_at)->format('F j, Y \a\t g:i A') }}
                            @endif
                        </td>
                    </tr>
                    <tr class="border-b border-teal-600">
                        <td class="py-2 px-3 font-medium border-r border-teal-600">Treatment Stage</td>
                        <td class="py-2 px-3">
                            {{ implode(', ', json_decode($individualpatientdetails->treatment_stage, true)) }}
                        </td>
                    </tr>
                    <tr class="border-b border-teal-600">
                        <td class="py-2 px-3 font-medium border-r border-teal-600">Insurance Status</td>
                        <td class="py-2 px-3">{{ $individualpatientdetails->insurance_status }}</td>
                    </tr>
                    <tr class="border-b border-teal-600">
                        <td class="py-2 px-3 font-medium border-r border-teal-600">Insurance Provider</td>
                        <td class="py-2 px-3">{{ $individualpatientdetails->insuranceProvider }}</td>
                    </tr>
                    <tr>
                        <td class="py-2 px-3 font-medium border-r border-teal-600">Policy Number</td>
        ...
        <div class="mt-6 p-4 bg-teal-800 rounded-lg overflow-x-auto">
            <h3 class="text-xl md:text-2xl font-semibold text-teal-300 mb-3">Financial & Employment</h3>
            <table class="w-full lg:w-fit table-auto border border-teal-600 text-teal-300 text-xs lg:text-sm">
                <tbody>
                    <tr class="border-b border-teal-600">
                        <td class="py-2 px-3 font-medium border-r border-teal-600">Employment Status</td>
                        <td class="py-2 px-3">{{ $individualpatientdetails->employment_status }}</td>
                    </tr>
                    <tr class="border-b border-teal-600">
                        <td class="py-2 px-3 font-medium border-r border-teal-600">Yearly Income</td>
                        <td class="py-2 px-3">{{ $individualpatientdetails->yearly_income }}</td>
                    </tr>
                    <tr class="border-b border-teal-600">
                        <td class="py-2 px-3 font-medium border-r border-teal-600">Income Source</td>
                        <td class="py-2 px-3">{{ $individualpatientdetails->income_source }}</td>
                    </tr>
                    <tr>
                        <td class="py-2 px-3 font-medium border-r border-teal-600">Emergency Contact</td>
        ...
        <!-- Secure Documents -->
        <div class="mt-6 p-4 bg-teal-800 rounded-lg">
            <h3 class="text-2xl font-semibold text-teal-300 mb-3">Secure Documents</h3>

            <div class="flex lg:flex-row flex-col lg:justify-around">
                <!-- Bank Statement -->
                <div class="mb-4">
                    <p class="text-teal-300 font-semibold">Bank Statement:</p>
                    <div class="w-full md:w-96 border-2 border-teal-500 text-white rounded-lg overflow-hidden">
                ...
                <!-- Government ID -->
                <div>
                    <p class="text-teal-300 font-semibold">Government ID:</p>
                    <div class="w-full md:w-96 border-2 border-teal-500 rounded-lg text-white overflow-hidden">
    ...
    <div class="w-full lg:w-2/4 mx-auto bg-teal-700 text-teal-300 my-6 p-6 rounded-lg shadow-md">
        ...
        <form action="{{ route('patients.update', $individualpatientdetails->id) }}" method="POST"
            ...
            {{-- shownavigators to switch --}}
            <div class="mb-4">
                <label class="block text-teal-300 font-semibold mb-1">Currently Assigned Navigator:</label>
                <div class="bg-slate-800 text-white p-2 rounded">
                    {{ $individualpatientdetails->navigator->name ?? 'Not Assigned' }}
                </div>
            </div>
            ...
            <div class="mb-4">
                <label class="block text-teal-300 font-semibold mb-1">Change Navigator:</label>
                <select name="navigatorId" id="navigatorId" class="w-full p-2 border text-black rounded-md">
                    <option value="" disabled
                        {{ old('navigatorId', $individualpatientdetails->navigatorId) ? '' : 'selected' }}>
                        -- Select Navigator --
                    </option>

                    @foreach($navigators as $navigator)
                        <option value="{{ $navigator->id }}"
                            {{ old('navigatorId', $individualpatientdetails->navigatorId) == $navigator->id ? 'selected' : '' }}>
            ...
            <!-- Treatment Closed -->
            <div>
                <label class="block font-medium">Treatment Closed</label>
                <select name="treatment_closed" class="w-full border rounded p-2 text-black">
                    <option value="0" {{ $individualpatientdetails->treatment_closed == 0 ? 'selected' : '' }}>No</option>
                    <option value="1" {{ $individualpatientdetails->treatment_closed == 1 ? 'selected' : '' }}>Yes</option>
                </select>
            </div>
