...
@section('title', '| FinancialAid')



@section('admincontent')

    <div class="container bg-teal-700 my-10 p-6 rounded-xl mx-auto border-teal-400">
        <h1 class="text-teal-300 font-bold text-2xl">Available Financial Aid Providers</h1>
        <p class="text-gray-200 text-sm"><em>Total Financial Aid Providers: {{ count($financialaid_details) }}</em></p>
        <a href="{{ route('testing') }}">open testing page</a>
        ...
        @if (count($financialaid_details) > 0)
            ...
            <div class="overflow-x-auto rounded">
                <table class="w-full text-center text-gray-900 rounded bg-gray-200 table-auto text-sm md:text-md mt-10">
                    <thead>
                        <tr class="text-sm md:text-md lg:text-md bg-slate-800 text-white border-b border-teal-400">
                            <th class="border border-teal-400 p-2">ID</th>
                            <th class="border border-teal-400 p-2">Name</th>
                            <th class="border border-teal-400 p-2">Contact</th>
                            <th class="border border-teal-400 p-2">Email</th>
                            <th class="border border-teal-400 p-2">Show Details</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($financialaid_details as $financialaid)
                            <tr class="hover:bg-gray-100">
                                <td class="border border-teal-400 p-2">{{ $financialaid->id }}</td>
                                <td class="border border-teal-400 p-2 font-semibold text-teal-600 hover:underline">
                                    <a
                                        href="{{ route('individualfinancialaiddetails', $financialaid->id) }}">{{ $financialaid->name }}</a>
                                </td>
                                <td class="border border-teal-400 p-2">{{ $financialaid->contact }}</td>
                                <td class="border border-teal-400 p-2">{{ $financialaid->email }}</td>
                                <td class="border border-teal-400 p-2">
                                    <button type="button"
                                        class="toggle-details bg-teal-600 hover:bg-teal-700 text-white py-1 px-3 rounded text-xs"
                            ...
                            <tr class="detail-row hidden" id="details-{{ $financialaid->id }}">
                                <td colspan="100%" class="bg-white border border-t-0 border-teal-600 p-4 text-left">
                                    <div class="flex flex-col md:flex-row items-start gap-4 text-sm text-gray-700">
                                        ...
                                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 flex-1">
                                            ...
                                            <div><strong>API URL:</strong><br>
                                                <a href="{{ $financialaid->apiurl }}" target="_blank"
                                                    class="text-blue-600 hover:underline">{{ $financialaid->apiurl }}</a>
                                            </div>
                                        </div>
                                    </div>
                                </td>
        ...
        @else
            <div class="mt-10 text-center">
                <p class="text-center rounded-md py-2 mt-2 text-sm bg-teal-600 text-gray-200">
                    <em>No Financial Aid Providers available! <a href="{{ route('addfinancialaid') }}"
                            class="font-semibold underline text-teal-300">Add New Financial Aid
                            Provider</a></em>
                </p>
            </div>
        @endif
