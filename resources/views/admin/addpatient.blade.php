...
@section('admincontent')
    ...
    <div class="max-w-4xl mx-auto bg-teal-700 p-8 rounded-lg my-10">
        <h2 class="text-2xl text-center font-bold underline text-teal-300">Patient Information Form</h2>
        ...
        @if($hospitals->isEmpty() or $navigators->isEmpty() or $cancertypes->isEmpty())
            <div class="bg-red-100 text-red-800 p-3 rounded-md mb-4 border-2 border-red-800">
                No hospitals/navigators/cancertypes are available! Admin Needs to Add these to add patient!
            </div>
        ...
        @else
            ...
            <form action="{{ route('show_addpatient') }}" method="POST" enctype="multipart/form-data"
                class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-10">
                @csrf

                <div>
                    <label class="block text-teal-300 font-semibold">Hospital</label>
                ...
                <div>
                    <label class="block text-teal-300 font-semibold">Navigator</label>
                    <select name="navigatorId" id="navigatorId" class="w-full p-2 border rounded-md"
                        onchange="updateHospitalName()">
                        <option value="" disabled {{ old('navigatorId') ? '' : 'selected' }}>-- Select Navigator --</option>

                        <!-- Dynamically populate options from the database -->
                        @foreach($navigators as $navigator)
                            <option value="{{ $navigator->id }}" data-name="{{ $navigator->name }}"
                                {{ old('navigatorId') == $navigator->id ? 'selected' : '' }}>
                                {{ $navigator->navigatorId }}, {{ $navigator->name }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    <label class="block text-teal-300 font-semibold">Patient Name</label>
                    <input type="text" name="name" class="w-full p-2 border rounded-md">
                </div>
                <div>
                    <label class="block text-teal-300 font-semibold">Doctor Name</label>
                    <input type="text" name="doctorname" class="w-full p-2 border rounded-md">
                </div>


                <div class="bg-teal-800 rounded-md">
                    <!-- Gender Label -->
                    <p class="text-teal-300 font-semibold mb-2 text-center">Gender</p>

                    <!-- Radio Buttons -->
                    <div class="flex items-center justify-center space-x-4">
                        <label class="flex items-center space-x-2 text-teal-300">
                            <input type="radio" name="gender" value="m" {{ old('gender') == 'm' ? 'checked' : '' }}
                                class="w-4 h-4 text-teal-300 border-teal-300 focus:ring-teal-300">
                            <span>Male</span>
                        </label>
                ...
                <div>
                    <label class="block text-teal-300 font-semibold">Date of Birth</label>
                    <input type="date" name="dateofbirth" class="w-full p-2 border rounded-md">
                </div>
                <div>
                    <label class="block text-teal-300 font-semibold">SSN</label>
                ...
                <div>
                    <label class="block text-teal-300 font-semibold">Password</label>
                ...
                <div>
                    <label class="block text-teal-300 font-semibold">City</label>
                ...
                <div>
                    <label class="block text-teal-300 font-semibold">Zip Code</label>
                    <input type="text" name="zipcode" class="w-full p-2 border rounded-md">
                </div>


                <div>
                    <label class="block text-teal-300 font-semibold">Cancer Type</label>
                ...
                <div class="mb-4 md:col-span-2">
                    <label for="treatment_stage" class="block text-md font-semibold text-teal-300">Treatment Stage</label>
                ...
                <div class="">
                    <label for="treatment_closed" class="block text-teal-300 font-semibold">Treatment Closed</label>
                    <select id="treatment_closed" name="treatment_closed" required
                        class="w-full p-2 border rounded-md focus:ring focus:ring-blue-300 bg-white text-teal-800">
                ...
                <div class="mb-4">
                    <label for="insurance_status" class="block text-md font-semibold text-teal-300">Insurance Status</label>
                ...
                <div class="mb-4">
                    <label for="employment_status" class="block text-md text-teal-300 font-semibold">Employment Status</label>
                ...
                <div>
                    <label class="block text-teal-300 font-semibold">Insurance Provider</label>
                    <input type="text" name="insuranceProvider" class="w-full p-2 border rounded-md">
                </div>

                <div>
                    <label class="block text-teal-300 font-semibold">Insurance Policy Number</label>
                    <input type="text" name="insurance_policy_number" class="w-full p-2 border rounded-md">
                </div>

                <div>
                    <label class="block text-teal-300 font-semibold">Yearly Income (USD$)</label>
                    <input type="number" name="yearly_income" class="w-full p-2 border rounded-md">
                </div>

                <div>
                    <label class="block text-teal-300 font-semibold">Income Source</label>
                    <input type="text" name="income_source" class="w-full p-2 border rounded-md">
                </div>

                <div>
                    <label class="block text-teal-300 font-semibold">Emergency Contact</label>
                    <input type="text" name="emergency_contact" class="w-full p-2 border rounded-md">
                </div>

                <div class="md:col-span-2 mt-3">
                    <label class="block text-teal-300 font-semibold">Profile Picture</label>
                ...
                <div class="md:col-span-2 mt-3">
                    <label class="block text-teal-300 font-semibold">Bank Statements</label>
                ...
                <div class="md:col-span-2">
                    <label class="block text-teal-300 font-semibold">Government ID (Driving License, Social Security
                        Card,
                        Passport)</label>
                    <em class="text-gray-200 text-sm">Accepted: pdf, jpg, png</em>
    ...
    <script>
        ...
        document.getElementById('hospitalId').addEventListener('change', function () {
            ...
            const navigatorSelect = document.getElementById('navigatorId');

            // Show loading message
            navigatorSelect.innerHTML = '<option disabled selected>Loading...</option>';

            // Fetch navigators for the selected hospital
            fetch(`/admin/hospital/${hospitalId}/navigators`)
                .then(response => response.json())
                .then(data => {
                    navigatorSelect.innerHTML = '<option disabled selected>-- Select Navigator --</option>';
                    data.forEach(navigator => {
                        const option = document.createElement('option');
                        option.value = navigator.id;
                        option.textContent = `${navigator.navigatorId}, ${navigator.name}`;
                        navigatorSelect.appendChild(option);
                    });
                })
                .catch(error => {
                    console.error('Error:', error);
                    navigatorSelect.innerHTML = '<option disabled selected>Error loading navigators</option>';
                });
        });
        ...
        //     // Show loading state
        //     cancerTypeSelect.innerHTML = '<option disabled selected>Loading cancer types...</option>';

        //     fetch(`/admin/hospital/${hospitalId}/cancer-types`)
        //         .then(response => response.json())
        //         .then(data => {
        //             cancerTypeSelect.innerHTML = '<option disabled selected>-- Select Cancer Type --</option>';
        //             data.forEach(cancer => {
        //                 const option = document.createElement('option');
        //                 option.value = cancer.name;
        //                 option.textContent = cancer.name;
        //                 cancerTypeSelect.appendChild(option);
        //             });
        //         })
        //         .catch(error => {
        //             console.error('Error fetching cancer types:', error);
        //             cancerTypeSelect.innerHTML = '<option disabled selected>Error loading cancer types</option>';
        //         });
        // });
