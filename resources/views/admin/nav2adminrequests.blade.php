{{-- this will show all the requests from navigator to admin, admin can mark pending, completed, cancelled --}}

@extends('admin.adminwelcome')

@section('title', '| Nav-Admin-Requests')


@section('admincontent')
    <div class="bg-teal-700 my-10 p-6 rounded-xl">
        <h2 class="text-2xl font-bold text-teal-300">Navigator Requests</h2>
        <em><span class="text-gray-300 text-sm">Total Navigator Requests: {{ count($requests) }}</span></em>
        ...
        @if(count($requests) > 0)
            ...
            <h2 class="text-xl font-bold mb-4 text-teal-300 mt-10 pb-2">Navigator → Admin Requests</h2>

            <!-- Scrollable table wrapper -->
            <div class="w-full overflow-x-auto">
                <table class="min-w-max w-full text-center border border-teal-700 text-xs">
                    <thead class="bg-slate-600 text-gray-200 text-xs uppercase">
                        <tr>
                            <th class="px-4 py-2">#</th>
                            <th class="px-4 py-2">Navigator</th>
                    ...
                    <tbody>
                        @foreach($requests as $index => $req)
                                <tr class="border-b border-teal-800 text-teal-900 bg-white hover:bg-teal-100">
                                    <td class="px-4 py-2">{{ $index + 1 }}</td>
                                    <td class="px-4 py-2">
                                        {{ $req->navigator->name ?? 'Unknown' }}
                                    </td>
                                    <td class="px-4 py-2">{{ $req->subject }}</td>
                                    <td class="px-4 py-2 capitalize">{{ $req->category }}</td>
                                    <td class="px-4 py-2">
                                        <span class="px-2 py-1 rounded text-white  {{ $req->priority === 'high' ? 'bg-red-500 ' :
                            ($req->priority === 'medium' ? 'bg-yellow-600 ' : 'bg-green-600 ') }}">
                                            {{ ucfirst($req->priority) }}
                                        </span>
                                    </td>
                                    <td class="px-4 py-2">
                                        <span class="px-2 py-1 rounded text-white {{ $req->status === 'pending' ? 'bg-yellow-600' :
                            ($req->status === 'in_progress' ? 'bg-blue-500' : 'bg-green-600 ') }}">
                                    ...
                                    <td class="px-4 py-2 flex items-center justify-center gap-2">
                                        ...
                                        <form action="{{ route('admin.navigator.requests.delete', $req->id) }}" method="POST"
                                            onsubmit="return confirm('Are you sure you want to delete this request?');" class="">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" title="Delete"
                                                class="text-red-400 bg-white rounded-full hover:text-red-300">
                                                <i class="bi bi-trash px-1 text-lg"></i>
                                            </button>
                                        </form>
                                    </td>
                                </tr>
                                ...
                                <tr id="details-{{ $req->id }}" class="hidden bg-teal-800/80 text-left text-white text-sm">
                                    <td colspan="8" class="px-4 py-3 border-t border-teal-600">
                                        ...
                                        <p class="flex flex-wrap text-sm mt-2"><a
                                                href="{{ route('admin.navigator.requests.edit', $req->id) }}"
        ...
        @else
            <div class="text-teal-300 text-sm bg-teal-900 p-2 text-center rounded-lg">
                <em>There is no any requests from Navigators!</em>
            </div>
        @endif
    </div>
@endsection