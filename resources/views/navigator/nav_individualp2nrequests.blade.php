...
@section('navigatorcontent')
    <div class="max-w-3xl mx-auto mt-10 bg-slate-800 p-6 rounded-lg shadow text-slate-100 text-sm">
        <h2 class="text-xl font-bold mb-4 border-b border-slate-500 pb-2">Update Patient Request</h2>
        ...
        <div class="overflow-x-auto text-sm">
            <table class="min-w-full table-auto border border-slate-600 rounded-lg shadow bg-slate-800 text-teal-200">
                <tbody class="divide-y divide-slate-600">
                    <tr>
                        <th class="text-left px-4 py-2 w-1/3 text-slate-300">Patient Name</th>
                        <td class="px-4 py-2">{{ $request->patient->name }}</td>
                    </tr>
                    <tr>
                        <th class="text-left px-4 py-2 text-slate-300">Patient ID</th>
                        <td class="px-4 py-2">{{ $request->patient->patientId }} ({{ $request->patient->id }})</td>
                    </tr>
                    <tr>
                        <th class="text-left px-4 py-2 text-slate-300">Hospital</th>
                        <td class="px-4 py-2">
                    ...
                    <tr>
                        <th class="text-left px-4 py-2 text-slate-300">Subject</th>
                        <td class="px-4 py-2">{{ $request->subject }}</td>
                    </tr>
                    <tr>
                        <th class="text-left px-4 py-2 text-slate-300">Category</th>
                        <td class="px-4 py-2">{{ $request->category }}</td>
                    </tr>
                    <tr>
                        <th class="text-left px-4 py-2 text-slate-300">Message</th>
                        <td class="px-4 py-2">{{ $request->message }}</td>
                    </tr>
                    <tr>
                        <th class="text-left px-4 py-2 text-slate-300">Priority</th>
                        <td class="px-4 py-2">
        ...
        ($request->priority === 'medium' ? 'bg-yellow-400 text-black' : 'bg-green-500 text-black') }}">
                    ...
                    <tr>
                        <th class="text-left px-4 py-2 text-slate-300">Status</th>
                        <td class="px-4 py-2">
                            <span class="px-2 py-1 rounded font-semibold text-xs
        ...
        <form action="{{ route('nav.patient.requests.update', $request->id) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
